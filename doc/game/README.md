...
`.timing` 文件是存关键时间点的文件，位宽为 40。

| 字数 |                             说明                             |
| :--: | :----------------------------------------------------------: |
|  1   |                     关键时间点数量 $T$。                     |
|  1   |     高 16 位：保留。低 24 位：歌曲持续时间，单位为毫秒。     |
| $T$  | 高 16 位：该时间点之后的移动速度，单位为 256 倍像素每毫秒。低 24 位：像素位置，单位为 256 倍像素。 |
